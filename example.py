...
if __name__ == '__main__':
    ...
    # instruction = "修一条铁路，宽度为30米，需要计算占用周边坡度小于15度的耕地面积。铁路数据是railway.shp，耕地数据是farmland.shp，地形数据是terrain.tif。" # 124954367.57385556
    ...
    # chain = init_gischain(llm="chatglm", key=os.environ.get("glm_key")) # 可以支持简单的指令
    # chain = init_gischain(llm="qwen-turbo", key=os.environ.get("qwen_key")) # 可以支持简单的指令
    # chain = init_gischain(llm="ErnieBot4", key={"ak":os.environ.get("wenxin_ak"),"sk":os.environ.get("wenxin_sk")} ) # 可以支持简单的指令

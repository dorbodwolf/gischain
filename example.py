...
if __name__ == '__main__':
    ...
    instruction = "修一条铁路，宽度为500米，需要计算占用周边的耕地面积。数据文件都在data目录下，铁路是railway.shp，耕地是land.shp。"
    # instruction = "修一条铁路，宽度为500米，需要计算占用周边坡度小于10度的耕地面积。数据文件都在data目录下，铁路是railway.shp，耕地是land.shp，地形数据是terrain.tif。"

    # 构造gischain，支持多种llm，基本都需要给出key
    # chain = init_gischain(llm="chatglm", key=glm_key) 
    # chain = init_gischain(llm="qwen-turbo", key=qwen_key) 
    chain = init_gischain(llm="gpt4", key=gpt_key) 
